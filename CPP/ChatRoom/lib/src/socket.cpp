...
#define LOG_ERROR(msg)          std::cout << msg << "\r"; \
                                exit(EXIT_FAILURE)
...
void Server::Binding() {
    ...
    std::cout << "Server: bind server socket done\r\n";
}
...
void Server::Listen() {
    ...
    std::cout << "Server: start listening to clients\r\n";
}

void Server::Init(const char *address=nullptr) {
    this->sockfd = socket(AF_INET, this->type, 0);
    if (this->sockfd < 0) {
        LOG_ERROR("Server: failed to create socket\r\n");
    }
    std::cout << "Server: create socket done\r\n";
...
int Server::Accept(sockaddr_in& client_addr, socklen_t& len) {
    int new_sockfd = accept(this->sockfd, (struct sockaddr *)&client_addr, &len);

    if (new_sockfd < 0) {
        std::cout << "Server: fail to connect to client\r\n";
        return 0;
    } 
    return new_sockfd;
}
